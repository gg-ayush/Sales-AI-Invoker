...
import { posts, pricingCards } from "@/constants/landing-page";
    ...
    <main>
      ...
      <section>
        <div className="flex items-center justify-center flex-col mt-[80px] gap-4">
          ...
          <p className="text-center max-w-[500px]">
            You AI powered sales assistant! Embed Invoker AI into any website 
      ...
      <section className="flex justify-center items-center flex-col gap-4 mt-10">
        ...
        <p className="text-muted-foreground text-center max-w-lg">
          Out straightforward pricing plans are tailored to meet your needs. If {"you're"} not ready to commit, you can start for free.
        </p>
      </section>
      <div className="flex justify-center gap-4 flex-wrap mt-6">
        {pricingCards.map((card) => (
          <Card 
            key={card.title}
            className={clsx('w-[300px] flex flex-col justify-between', {'border-2 border-primary': card.title === 'Unlimited'})}
          >
            <CardHeader>
      ...
      <section className="lg:grid-cols-3 grid-cols-1 grid gap-5 container">
        {
          posts && posts.map((post) => (<Link href={`/blogs/${post.id}`} key={post.id}>
