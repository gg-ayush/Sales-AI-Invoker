...
const Messenger = (props: Props) => {
    const { messageWindowRef, register, onHandleSentMessage, chats, loading, chatRoom } = useChatWindow()
  return (
    <div className='flex-1 flex flex-col h-0 relative'>
        <div className='flex-1 h-0 w-full flex flex-col'>
            <Loader loading={loading}>
                <div className='w-full flex-1 h-0 flex flex-col gap-3 pl-5 py-5 chat-window overflow-y-auto'>
                    {
                        chats.length ? (
                            chats.map((chat) => (
                                <Bubble 
