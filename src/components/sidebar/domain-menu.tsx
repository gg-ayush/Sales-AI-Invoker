      ...
      {domains &&
          domains.map((domain) => (
            <Link
              ...
              key={domain.id}
              className={cn(
                'flex gap-3 hover:bg-white dark:hover:bg-slate-900 justify-center rounded-full transition duration-100 ease-in-out cursor-pointer ',
                !min ? 'p-2' : 'py-2',
                domain.name.split('.')[0] == isDomain && 'bg-white dark:bg-slate-900'
              )}
            >
              <Image
